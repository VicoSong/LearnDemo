...
import com.swk.librarymanagerdemo.Compz.MapPz;
...
import com.swk.librarymanagerdemo.util.MyDocument;

public class BookBackIFrame extends JInternalFrame {
	private Operator user =Login.getUser(); 
	private JTable table;
	private JTextField operator;
	private JTextField todaydate;
	private JTextField finemoney;
	...
	public final void add() {
		...
		List list=SQLMethod.selectBookBack(readerISBNs);
		for(int i=0;i<list.size();i++){
			BookReturn back=(BookReturn)list.get(i);
			id=	back.getId();
	...
	class TableListener extends MouseAdapter {
		public void mouseClicked(final MouseEvent e) {
			...
			List list =SQLMethod.selectBookTypeFk(table.getValueAt(selRow, 2).toString().trim());
			for(int i=0;i<list.size();i++){
				BookType booktype=(BookType)list.get(i);
				fine=booktype.getFinePerDay();
			...
			if(days3>0){
				...
				Double totalFine=Double.valueOf(finePerDay)*days3;
				finemoney.setText(zfk+"元");
			}
			else{
