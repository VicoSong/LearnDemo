...
import com.swk.librarymanagerdemo.Compz.Item;
import com.swk.librarymanagerdemo.dao.SQLMethod;
import com.swk.librarymanagerdemo.model.BookType;
import com.swk.librarymanagerdemo.util.CreatecdIcon;
...
public class BookAddIFrame extends JInternalFrame {
	...
	public BookAddIFrame() {
		...
		for(int i=0;i<list.size();i++){
			...
			Item item=new Item();
			item.setId((String)booktype.getBookTypeID());
	...
	class ISBNFocusListener extends FocusAdapter {
		public void focusLost(FocusEvent e){
			if(!SQLMethod.selectBookInfo(ISBN.getText().trim()).isEmpty()){
	...
	class addBookActionListener implements ActionListener {		// 添加按钮的单击事件监听器
		public void actionPerformed(final ActionEvent e) {
			...
			int i=SQLMethod.Insertbook(ISBNs,bookTypes, bookNames, writers, translators, publishers, java.sql.Date.valueOf(pubDates),Double.parseDouble(prices));
