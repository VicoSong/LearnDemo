...
<body onunload="quit.jsp">
	<table width="755" border="0" align="center" cellpadding="0"
		cellspacing="0" bordercolor="#FFFFFF" bordercolordark="#000000"
		...
		<tr>
			<td width="24%" height="390" valign="top"><jsp:include
					page="../../leftManager.jsp" /></td>
			<td width="76%" height="390" bgcolor="#FFFFFF">
				<div align="center">
					<table width="96%" height="16" border="0" cellpadding="0"
						cellspacing="0">
						<tr>
							<td bgcolor="#E6E6E6"><div align="center" class="dhz">
									<font color="#990000"><b>商品订单信息查询</b></font>
								</div></td>
						</tr>
					</table>
      ...
      OrderForm form=(OrderForm)list.get(i);
          %>
						<tr align="center" class="zi">
							...
							<td class="linkBlack">
								<%if(form.getSign().equals("0")){%> <a
								href="orderAction.do?action=0&sign=<%=form.getSign()%>">否</a> <%}else{%>
								<a href="orderAction.do?action=0&sign=<%=form.getSign()%>">是</a>
								<%}%>
							</td>
							<td><%=form.getCreaTime()%></td>
							<td class="linkBlack"><a
								href="orderAction.do?action=3&number=<%=form.getNumber()%>">详细信息</a>
								&nbsp;&nbsp; <%if(form.getSign().equals("1")){%> 已出 <%}else if(request.getParameter("sign")==null){%>
								<a href="orderAction.do?action=1&number=<%=form.getNumber()%>">出货</a>
								<%}else{%> <a
								href="orderAction.do?action=1&sign=<%=request.getParameter("sign")%>&number=<%=form.getNumber()%>">出货</a>
								<%}%> &nbsp;&nbsp; <a
								href="javascript:deleteOrder('<%=form.getNumber()%>')">删除</a></td>
						</tr>
						<%}%>
					</table>
					<p>&nbsp;</p>
					...
					<table width="96%" border="0" align="center" cellpadding="0"
						...
						<tr align="center" class="zi">
							...
							<td width="19%" class="linkBlack">
								<%if((number+1)==1){%> 上一页<%}else{%><a
								href="orderAction.do?action=0&i=<%=number-1%>">上一页</a>
							</td>
							<%}%>
							<td width="16%" class="linkBlack">
								<%if(maxPage<=(number+1)){%>下一页<%}else{%><a
								href="orderAction.do?action=0&i=<%=number+1%>">下一页</a>
							</td>
							<%}%>
						</tr>
					</table>
					...
					<table width="96%" border="0" align="center" cellpadding="0"
						cellspacing="0">
						<tr align="center" class="zi">
							<td width="14%">共为<%=maxPage%>页
							</td>
							<td width="18%">共有<%=pageNumber%>条记录
							</td>
							<td width="22%">当前为第<%=number+1%>页
							</td>
							<td width="16%">
								<%if((number+1)==1){%> 上一页<%}else{%><a
								href="orderAction.do?action=0&i=<%=number-1%>&sign=<%=request.getParameter("sign")%>">上一页</a>
							</td>
							<%}%>
							<td width="12%">
								<%if(maxPage<=(number+1)){%>下一页<%}else{%><a
								href="orderAction.do?action=0&i=<%=number+1%>&sign=<%=request.getParameter("sign")%>">下一页</a>
							</td>
							<%}%>
							<td width="18%" class="linkBlack"><a
								href="orderAction.do?action=0">返回</a></td>
						</tr>
					</table>
					<%}%>
				</div>
		...
		<tr bgcolor="#FFFFFF">
			<td height="50" colspan="2"><p align="center"><jsp:include
